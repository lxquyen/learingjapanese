...
import com.quinny898.library.persistentsearch.sharedpreferences.ConfigStoreManager;
...
public class SearchBox extends RelativeLayout {
    ...
    private ConfigStoreManager<SearchResult> configStoreManager;
    private final static String KEY_RESULT = "key_result_" + SearchBox.class.getSimpleName();
    private final static String KEY_RESULT_1 = "key_result_1" + SearchBox.class.getSimpleName();
    ...
    public SearchBox(final Context context, AttributeSet attrs, int defStyle) {
        ...
        configStoreManager = new ConfigStoreManager<SearchResult>(context);
        this.searchOpen = true;
        this.isMic = true;
        this.materialMenu = (MaterialMenuView) findViewById(R.id.material_menu_button);
        materialMenu.setVisibility(GONE);

        mOverView = (RelativeLayout) findViewById(R.id.over_view);
        this.img_back = (ImageView) findViewById(R.id.img_back);
        card_view=(RelativeLayout) findViewById(R.id.card_view);
        img_back.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mContainerSupportFragment != null) {
                    mContainerSupportFragment.getActivity().onBackPressed();
                } else if (mContainerActivity != null) {
                    mContainerActivity.onBackPressed();
                }
            }
        });
...
//        search.requestFocus();
        ...
        this.results.setOnItemClickListener(new OnItemClickListener() {
...
//                setSearchString(result.title);
        ...
        if (mContainerSupportFragment != null) {
            resultList.addAll(configStoreManager.getArrayList(KEY_RESULT));
        }else
        {
            resultList.addAll(configStoreManager.getArrayList(KEY_RESULT_1));
        }
        ...
        if (mContainerSupportFragment != null) {
            ...
            searchables.addAll(configStoreManager.getArrayList(KEY_RESULT));
        }else
        {

            searchables.addAll(configStoreManager.getArrayList(KEY_RESULT_1));
        }
    ...
    public void setEditText(String text,int color)
    {
        if (search!=null)
        {
            search.setHint(text);
            search.setHintTextColor(context.getResources().getColor(color));
            search.setTextColor(context.getResources().getColor(color));
        }
    ...
    public void setKeyResult(String result)
    {
    ...
    public void setImageMic(int id)
    {
...
//            mOverView.setVisibility(View.GONE);
    ...
    public String getSearchText() {
        return search.getText().toString().trim(); //quanpv add trim()
    }
    ...
    private void addResultToTop(SearchResult result) {
        if (resultList != null) {
...
//        if (!searchables.contains(searchable)) {
        for (SearchResult searchResult : searchables) {
            if (searchResult.toString().equals(searchable.toString())) {
                searchables.remove(searchResult);
                searchables.add(0, searchable);
                if (mContainerSupportFragment != null) {

                    configStoreManager.putArrayList(KEY_RESULT, searchables);
                }else
                {

                    configStoreManager.putArrayList(KEY_RESULT_1, searchables);
                }
                return;
            }
        }
...
//            mOverView.setVisibility(GONE);
    ...
    private void search(String text) {
        SearchResult option = new SearchResult(text, 0);
        search(option, false);
